	...
	window.base = {
		...
		initNavToggleSubmenus: function() {
			...
			for ( let i = 0; i < navTOGGLE.length; i++ ) {
				window.base.initEachNavToggleSubmenu( navTOGGLE[ i ] );
				window.base.initEachNavToggleSubmenuInside( navTOGGLE[ i ] );
			}
		},
			...
			for ( let i = 0; i < SUBMENUS.length; i++ ) {
				...
				if ( dropdown ) {
					...
					parentMenuItem.addEventListener( 'mouseleave', function( e ) {
						window.base.toggleSubMenu( e.target, false );
					} );
					...
					SUBMENUS[ i ].addEventListener( 'keydown', function( e ) {
						...
						if ( 9 === e.keyCode ) {
							if ( e.shiftKey ) {
								...
								if ( window.base.isfirstFocusableElement (SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
									window.base.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
								}
								// Means we're tabbing out of the end of the submenu.
							} else if ( window.base.islastFocusableElement( SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
								window.base.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
							}
						}
			...
			for ( let i = 0; i < SUBMENUPARENTS.length; i++ ) {		
				...
				SUBMENUPARENTS[ i ].addEventListener( 'mouseenter', function( e ) {
					if ( SUBMENUPARENTS[ i ].querySelector( 'ul.sub-menu' ) ) {
						...
						var off = window.base.getOffset( elm );
						var l = off.left;
			...
			*/
			if ( parentMenuItemToggled ) {
				...
				for ( let i = 0; i < subMenuItemsToggled.length; i++ ) {
					window.base.toggleSubMenu( subMenuItemsToggled[ i ], false );
				}
			} else {
				...
				for ( let i = 0; i < parentMenuItemsToggled.length; i++ ) {
					window.base.toggleSubMenu( parentMenuItemsToggled[ i ], false );
				}
					...
					if ( toggle.dataset.setFocus && changeFocus ) {
						...
						if ( focusElement ) {
							...
							if ( focusElement.hasAttribute( 'aria-expanded') ) {
								window.base.toggleAttribute( focusElement, 'aria-expanded', 'true', 'false' );
							}
						}
					}
				}, duration);
		...
		initToggleDrawer: function() {
			...
			for ( let i = 0; i < drawerTOGGLE.length; i++ ) {
				drawerTOGGLE[ i ].addEventListener('click', function( event ) {
					...
					window.base.toggleDrawer( drawerTOGGLE[ i ] );
				} );
			}
				...
				if ( event.keyCode === 27 ) {
					if ( document.querySelectorAll( '.popup-drawer.show-drawer.active' ) ) {
						...
						document.querySelectorAll( '.popup-drawer.show-drawer.active' ).forEach(function ( element ) {
							window.base.toggleDrawer( document.querySelector('*[data-toggle-target="' + element.dataset.drawerTargetString + '"]' ) );
						} );
					}
				}
			  });
			...
			document.addEventListener( 'click', function (event) {
				...
				if ( target === modal ) {
					window.base.toggleDrawer(document.querySelector('*[data-toggle-target="' + modal.dataset.drawerTargetString + '"]'));
				}
		...
		initMobileToggleSub: function() {
			...
			modalMenus.forEach( function( modalMenu ) {
				...
				if ( activeMenuItem ) {
					window.base.findParents( activeMenuItem, 'li' ).forEach( function( element ) {
						var subMenuToggle = element.querySelector( '.drawer-sub-toggle' );
						if ( subMenuToggle ) {
							window.base.toggleDrawer( subMenuToggle, true );
						}
					} );
				}
			} );
			...
			for ( let i = 0; i < drawerSubTOGGLE.length; i++ ) {
				drawerSubTOGGLE[ i ].addEventListener('click', function (event) {
					event.preventDefault();
					window.base.toggleDrawer( drawerSubTOGGLE[ i ] );
				} );
			}
		},
		/**
		...
		initMobileToggleAnchor: function() {
			...
			for ( let i = 0; i < menuLink.length; i++ ) {
				menuLink[ i ].addEventListener('click', function (event) {
					window.base.toggleDrawer( mobileModal.querySelector( '.menu-toggle-close' ), false );
				} );
			}
		},
		/**
		...
		initTransHeaderPadding: function() {
			...
			var updateHeroPadding = function( e ) {
				header
				if ( baseConfig.breakPoints.desktop <= window.innerWidth ) {
		...
		initStickyHeader: function() {
			var desktopSticky = document.querySelector( '#main-header .base-sticky-header' ),
				...
				lastScrollTop = 0,
				activeOffsetTop = 0;
				if ( parseInt( baseConfig.breakPoints.desktop ) < window.innerWidth ) {
					activeSize = 'desktop';
					if ( desktopSticky ) {
						desktopSticky.style.position = 'static';
						activeOffsetTop = window.base.getOffset( desktopSticky ).top;
						desktopSticky.style.position = null;
					}
				} else {
					if ( mobileSticky ) {
						mobileSticky.style.position = 'static';
						activeOffsetTop = window.base.getOffset( mobileSticky ).top;
						mobileSticky.style.position = null;
					}
				}
				var updateSticky = function( e ) {
					var activeHeader;
					var offsetTop = window.base.getOffset( wrapper ).top;
					if ( document.body.classList.toString().includes( 'boom_bar-static-top' ) ) {
						var boomBar = document.querySelector( '.boom_bar' );
						offsetTop = window.base.getOffset( wrapper ).top - boomBar.offsetHeight;
					}
					if ( proElements.length ) {
						...
						offsetTop = window.base.getOffset( wrapper ).top - proElementOffset;
					}
					if ( proSticky.length ) {
						var proOffset = 0;
						...
						offsetTop = window.base.getOffset( wrapper ).top + proOffset;
					}
					if ( baseConfig.breakPoints.desktop <= window.innerWidth ) {
					...
					if ( baseConfig.breakPoints.desktop <= window.innerWidth ) {
						if ( activeSize === 'mobile' ) {
							activeOffsetTop = window.base.getOffset( activeHeader ).top;
							activeSize = 'desktop';
				...
				if ( desktopSticky || mobileSticky ) {
					...
					if ( document.body.classList.contains( 'woocommerce-demo-store' ) && document.body.classList.contains( 'base-store-notice-placement-above' ) ) {
		...
		getTopOffset: function( event = 'scroll' ) {
			if ( event === 'load' ) {
				...
				var desktopSticky = document.querySelector( '#main-header .base-sticky-header:not([data-reveal-scroll-up="true"])' ),
					mobileSticky  = document.querySelector( '#mobile-header .base-sticky-header:not([data-reveal-scroll-up="true"])' );
			}
			...
			return Math.floor( activeScrollOffsetTop + activeScrollAdminOffsetTop + Math.floor( baseConfig.scrollOffset ) );
		},
		scrollToElement: function( element, history, event = 'scroll' ) {
			history = (typeof history !== 'undefined') ? history : true;
			var offsetSticky = window.base.getTopOffset( event );
		...
		anchorScrollToCheck: function( e, respond ) {
			...
			e.preventDefault();
			window.base.scrollToElement( targetAnchor );
		},
		/**
		...
		initStickySidebarWidget: function() {
			...
			var offsetSticky = window.base.getTopOffset(),
			...
			if ( window.location.hash != '' ) {
				...
				if ( element ) {
					window.setTimeout( function() {
						window.base.scrollToElement( element, false, 'load' );
					}, 100 );
				}
			}
